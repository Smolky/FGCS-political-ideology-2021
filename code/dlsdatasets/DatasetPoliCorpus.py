...
class DatasetPoliCorpus (Dataset):
    ...
    def compile (self):
        ...
        self.df \
            ...
            .to_csv (self.get_working_dir ('helers', 'stats-politics-by-month.csv'))
        ...
        # Get dataframe for task 2, author identification, by selecting those authors from task 1 (training)
        df_task2 = self.df.loc[self.df['user'].isin (df_task1_train['user'].unique ())]
        
        
        # Sample `tweets_min_limit` tweets per user
        df_task2_train = df_task2.groupby ('user', group_keys = False).apply (lambda x: x.sample (min (len (x), self.tweets_min_limit)))
        
        
        # For validation and testing the tweets are those not selected from training
        df_task2_val_test = df_task2.loc[~df_task2['twitter_id'].isin (df_task2_train['twitter_id'])]
        
        
        # Get the IDs of the rest of the tweets and divide them into two pieces
        rest_of_the_tweets = pd.Series (df_task2_val_test['twitter_id']).sample (frac = 0.5, random_state = bootstrap.seed)
        
        
        # Obtain validation and training using the remaining IDs that were not selected for training
        df_task2_val = df_task2_val_test.loc[df_task2_val_test['twitter_id'].isin (rest_of_the_tweets)]
        df_task2_test = df_task2_val_test.loc[~df_task2_val_test['twitter_id'].isin (rest_of_the_tweets)]
